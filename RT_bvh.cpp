...
void RTDynamicBVH::traverse(const RTNode& node)const
{
    ...
    for(uint32_t i = 0; i < node.faces; i++)
       {
        ...
        if(model->meshlist[face.mesh_index].params.optional[1] == false) continue; // not visible

        // must have shader
        auto shader = model->meshlist[face.mesh_index].params.shader;
        if(!shader) continue;
        ...
        if(result.intersect) {
           ...
           auto sample = (*shader)(model->meshlist[face.mesh_index], face.refs, test, result.u, result.v, w);
           dt->Update(sample, result.t);
          }
       }
   }
 ...
 // note we need near and far plane information here
    ...
    if(ray3D_AABB(test, node.bbmin, node.bbmax, 0.0f, 1000.0f)) traverse(tree[child1]);
    if(ray3D_AABB(test, node.bbmin, node.bbmax, 0.0f, 1000.0f)) traverse(tree[child2]);
   }
}

bool RTDynamicBVH::raytest(const ray3D& ray)
{
